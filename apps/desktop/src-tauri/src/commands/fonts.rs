...
fn enumerate_system_fonts() -> MoonSnapResult<Vec<String>> {
    let source = SystemSource::new();

    let families = source
        .all_families()
        .map_err(|e| MoonSnapError::Other(format!("Failed to get font families: {}", e)))?;
...
#[tauri::command]
pub async fn get_system_fonts() -> MoonSnapResult<Vec<String>> {
...
#[tauri::command]
pub fn get_font_weights(family: String) -> MoonSnapResult<Vec<u32>> {
    let source = SystemSource::new();
