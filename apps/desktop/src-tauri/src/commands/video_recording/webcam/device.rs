...
fn query_supported_resolutions(
    ...
    // Find max resolution (capped at 1080p for performance)
    let (raw_max_width, raw_max_height) = formats
        .iter()
        .map(|f| (f.width(), f.height()))
        .max_by_key(|(w, h)| (*w as u64) * (*h as u64))
        .unwrap_or((1280, 720));

    // Cap at 1080p for smooth preview performance
    let max_width = raw_max_width.min(1920);
    let max_height = raw_max_height.min(1080);
    ...
    log::debug!(
        "[WEBCAM] Device '{}' resolutions: 1080p={} 720p={} 480p={} max={}x{} (raw={}x{})",
        device.name().to_string_lossy(),
        supports_1080p,
        supports_720p,
        supports_480p,
