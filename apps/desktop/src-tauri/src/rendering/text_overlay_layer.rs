...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    if (typewriter_active > 0.5) {
        ...
        if (uv.y >= last_v_top && uv.y < last_v_bottom) {
            // On the partial line — clip horizontally
...
/// Per-quad rendering parameters, computed by `PreRenderedTextStore::get_gpu_quads_for_frame`.
pub struct TextOverlayQuad {
    ...
    /// Segment index → GPU texture lookup.
