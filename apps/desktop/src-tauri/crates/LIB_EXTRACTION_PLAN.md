# SnapIt Rust Library Extraction Plan

## Goal

Turn SnapIt's Rust backend into reusable crates that can be consumed by future capture/editor products, while keeping the Tauri app as a thin integration shell.
...
- Explicit crate roots + smoke tests + per-crate README for `snapit-core`, `snapit-domain`, `snapit-media`, `snapit-render`.
- Wildcard crate-root exports tightened in `snapit-core` and `snapit-media`.
- Phase 2 started:
- New `snapit-capture` crate added and wired into workspace/app dependencies.
- New `snapit-export` crate added and wired into workspace/app dependencies.
- Moved timing/state modules from app shell to `snapit-capture`:
  - `timestamp`
  - `master_clock`
  - `state`
- Moved recorder support modules from app shell to `snapit-capture`:
  - `recorder/buffer`
  - `recorder/helpers`
  - `d3d_capture`
  - `recorder/capture_source`
- Moved audio/runtime modules from app shell to `snapit-capture`:
  - `audio` (legacy cpal capture helpers)
  - `audio_wasapi`
  - `audio_sync`
  - `audio_multitrack`
  - runtime monitor/display helpers from `video_recording/types.rs`
- Moved GIF engine modules from app shell to `snapit-capture`:
  - `gif_encoder`
  - `ffmpeg_gif_encoder`
- Moved additional runtime modules from app shell to `snapit-capture`:
  - `fragmentation` (manifest + atomic persistence helpers)
  - `desktop_icons`
- Moved recorder orchestration module (Tauri-free core):
  - `recorder/gif` moved to `snapit-capture::recorder_gif` with callback-based state emission
- Moved shared video-capture planning helpers:
  - `recorder/video` mode-to-source planning extracted to `snapit-capture::recorder_video_capture`
...
- Moved shared cursor region resolution helpers:
  - `recorder/video` cursor-region mode handling extracted to `snapit-capture::recorder_cursor_region`
  - app now injects only lookup adapters (`get_window_rect`, `get_scap_display_bounds`)
- Moved shared audio artifact path planning:
  - quick-capture/editor WAV path decisions extracted to `snapit-capture::recorder_audio_paths`
- Moved shared loop command/pause control:
  - MP4 recorder command channel + pause/resume state logic extracted to `snapit-capture::recorder_loop_control`
...
- Moved shared first-frame synchronization:
  - stale-frame skip + first post-start frame selection extracted to `snapit-capture::recorder_first_frame`
  - app recorder now delegates initial timestamp alignment to crate helper
- Moved shared frame pacing helpers:
  - frame interval sleep computation extracted to `snapit-capture::recorder_pacing`
  - app recorder now delegates deterministic pacing math to crate helper
- Moved shared finalization planning helpers:
  - quick-capture vs editor finalization decisions extracted to `snapit-capture::recorder_finalization`
  - project artifact flag computation moved from app recorder into crate helper
- Moved shared progress emission helpers:
  - periodic `RecordingState::Recording` construction/cadence moved to `snapit-capture::recorder_progress`
  - app recorder now provides only callback emission wiring
- Moved shared output-path planning helpers:
  - editor/quick-capture screen+webcam path planning moved to `snapit-capture::recorder_output_paths`
- Moved shared cursor persistence callback helper:
  - cursor persistence gating+callback moved to `snapit-capture::recorder_cursor_persistence`
- Moved shared webcam feed probing helper:
  - webcam dimension polling/wait logic moved to `snapit-capture::recorder_webcam_feed`
  - callback-based feed start + probe flow moved behind `prepare_webcam_feed(...)`
- Moved shared webcam encoder lifecycle helpers:
  - conditional start + finalize/cancel flow moved to `snapit-capture::recorder_webcam_lifecycle`
  - app recorder now injects only encoder construction/finalize callbacks
- Moved shared video postprocess helper:
  - non-empty verification + optional audio mux + faststart flow moved to `snapit-capture::recorder_video_postprocess`
- Moved shared MP4 capture loop orchestration:
  - command/pause control, max-duration gating, pacing sleep, first-frame callback, and periodic recording-state emission moved to `snapit-capture::recorder_video_loop::run_video_capture_loop`
...
- Moved shared MP4 finalization orchestration:
  - post-recording cursor persistence gating + encoder finish + mux/faststart + project-file creation flow moved to `snapit-capture::recorder_video_finalize::finalize_video_capture`
...
- Moved shared capture-thread lifecycle orchestration:
  - post-capture cancel/success/error result handling centralized in `snapit-capture::recorder_capture_lifecycle::finalize_capture_thread_result`
  - panic payload normalization + error propagation centralized in `snapit-capture::recorder_capture_lifecycle::handle_capture_thread_panic`
...
- Moved shared capture-thread panic boundary + lifecycle wrapper:
  - centralized catch-unwind + cancellation-resolution + callback dispatch in `snapit-capture::recorder_capture_lifecycle::run_capture_thread_with_lifecycle`
...
- Moved shared capture-thread spawn + setup/teardown wrapper:
  - added `snapit-capture::recorder_capture_lifecycle::spawn_capture_thread_with_lifecycle`
...
- Moved shared countdown orchestration:
  - start-countdown delay + per-second ticks + final cancel gate centralized in `snapit-capture::recorder_countdown::run_recording_countdown`
...
- Moved recorder command-state transition logic:
  - stop/cancel/pause/resume validation + command dispatch centralized in `snapit-capture::state::RecordingController::{request_stop, request_cancel, request_pause, request_resume}`
...
- Reduced app-shell compatibility shims in recorder path:
  - `recorder/video.rs` now imports `snapit-capture` modules directly (`audio_multitrack`, `frame_buffer`, `recorder_helpers`, `state`, `timestamp`)
  - removed obsolete recorder shim files: `recorder/buffer.rs`, `recorder/capture_source.rs`, `recorder/helpers.rs`
  - `recorder/mod.rs`, `recorder/gif.rs`, and `webcam/segmented.rs` now import `snapit-capture` modules directly (`desktop_icons`, `state`, `fragmentation`)
  ...
  - app startup desktop-icon safety hooks in `src/lib.rs` now call `snapit_capture::desktop_icons::*` directly
  ...
  - normalized command-layer type imports to shared crates:
    - removed `commands/captions` type re-export surface; `commands/captions/mod.rs` now uses `snapit_domain::captions::*` internally and downstream modules import caption DTOs directly from `snapit_domain`
    - removed `commands/capture` type re-export surface; `commands/capture/mod.rs` + `commands/capture/fallback.rs` now use `snapit_domain::capture` directly and call sites import `ScreenRegionSelection` from domain crate
    - `commands/storage/operations.rs` + `commands/storage/tests.rs` now consume `snapit_domain::storage` directly (no `storage/types.rs` shim)
  - removed obsolete storage FFmpeg shim:
    - deleted `commands/storage/ffmpeg.rs`
    - app call sites now use `snapit_media::ffmpeg::{find_ffmpeg, find_ffprobe, create_hidden_command}` directly
    - removed `commands/storage::find_ffmpeg/find_ffprobe` re-export from `commands/storage/mod.rs`
  - collapsed thin rendering shim modules into direct `snapit-render` imports:
    - app imports in `rendering/compositor.rs`, `rendering/editor_instance.rs`, `rendering/exporter/mod.rs`, `preview/mod.rs`, `preview/native_surface.rs`, and caption parity/pixel tests now use `snapit_render::*` directly
    - deleted obsolete wrapper files:
      ...
      - `rendering/types.rs`
      - `rendering/zoom.rs`
    - final `rendering/mod.rs` compatibility facade removed for shared crate types; downstream modules now import `snapit_render::*` directly where needed
  - removed exporter-level compatibility re-exports where not needed:
    - `rendering/editor_instance.rs` now imports webcam overlay helpers directly from `snapit_render::webcam_overlay`
    - `rendering/exporter/tests.rs` now imports `build_webcam_overlay` directly from `snapit_render::webcam_overlay`
    - removed re-exports from `rendering/exporter/mod.rs` for:
      - `snapit_export::frame_ops::draw_cursor_circle`
      - `snapit_render::webcam_overlay::{build_webcam_overlay, is_webcam_visible_at}`
  ...
  - app shell shim count reduced further:
    - `rg "^pub use snapit_" apps/desktop/src-tauri/src` now reports no matches (0 remaining direct crate re-export wrappers)
  - removed core error shim module:
    - deleted `src/error.rs` (`pub use snapit_core::error::*`)
    - app now imports `SnapItResult` directly from `snapit_core::error` (e.g., `config/webcam.rs`)
    - removed `pub mod error;` from `src/lib.rs`
  - narrowed parity wrapper surface:
    - `rendering/parity.rs` now imports parity types internally instead of re-exporting `snapit_render::parity::*`
    - parity tests now consume `snapit_render::parity` directly for shared layout/math helpers
  - removed config-level webcam type re-export:
    - `config/webcam.rs` now imports webcam enums/types from `snapit_domain::webcam` privately
    - `commands/video_recording/mod.rs` now re-exports `WebcamShape`/`WebcamSize` directly from `snapit_domain::webcam` while keeping runtime config access (`get_webcam_settings`, `WEBCAM_CONFIG`) from app config module
  - removed webcam-module domain re-export surface:
    - `commands/video_recording/webcam/mod.rs` no longer re-exports `snapit_domain::webcam::{compute_webcam_rect, WebcamPosition, WebcamSettings, WebcamShape, WebcamSize}`
  ...
  - narrowed video-project wrapper surface:
    ...
    - downstream consumers now import `snapit_domain::video_project::*` directly where needed
  ...
  - narrowed top-level video-recording shim exports:
    ...
    - `config/recording.rs` now imports `GifQualityPreset` directly from `snapit_capture::ffmpeg_gif_encoder`
- Started export extraction in `snapit-export`:
  - moved encoder quality/hardware selection primitives into `snapit-export::encoder_selection`
  - app exporter `encoder_selection.rs` now acts as runtime probe adapter
  - moved FFmpeg audio/filter/quality planning into `snapit-export::ffmpeg_plan`
  - added complete FFmpeg encoder-argument planning API (`build_encoder_args`) in `snapit-export::ffmpeg_plan`
  - app exporter `ffmpeg.rs` now acts as process-launch adapter for crate-built argument plans
  - moved caption source-time to timeline-time remapping into `snapit-export::caption_timeline`
  - moved export crop/composition dimension planning into `snapit-export::composition_plan`
  - moved export timeline/decode planning (durations, decode window, frame/time conversion, segment skip checks) into `snapit-export::timeline_plan`
  - moved NV12 fast-path compatibility + per-frame NV12/RGBA path decisions into `snapit-export::frame_path_plan`
  - added aggregated project-level export planning API (`snapit-export::export_plan`) and switched app exporter to consume it
  - moved decode stream input/window planning (screen/webcam path selection + decode range/frame window) into `snapit-export::decode_plan`
  - moved per-frame RGBA/NV12 crop normalization decision logic into `snapit-export::frame_path_plan`
  - moved per-frame base render branch + webcam overlay decision logic into `snapit-export::frame_path_plan` (`plan_frame_render`)
  - moved export cancellation token + render-stage progress mapping into `snapit-export::job_control`
  - app exporter now consumes crate cancel token and crate render progress mapping
  - moved FFmpeg stderr tail extraction helper into `snapit-export::job_control::tail_lines`
  - moved export-global cancel-token ownership into `snapit-export::job_control` (`export_cancel_token`, `request_cancel_export`, `reset_cancel_export`, `is_export_cancelled`)
  - moved generic decode/encode channel pipeline runtime into `snapit-export::pipeline`
  - app exporter `pipeline.rs` now acts as adapter around `StreamDecoder` and FFmpeg stdin
  - added callback-driven export loop-control runner `snapit-export::job_runner`
  - app exporter now delegates cancel/target stop checks, progress cadence callbacks, and drain gating to crate runner
  - moved generic embedded-asset temp-file staging into `snapit-export::temp_file`
  - app FFmpeg adapter now delegates typewriter-loop WAV staging to crate helper
  - moved FFmpeg audio-input request assembly (path presence + typewriter-window gating + staged loop path callback) into `snapit-export::ffmpeg_plan::prepare_audio_input_request`
  - added async callback-driven decode loop APIs `snapit-export::export_job::{run_export_loop, run_export_loop_with_context}`
  - app exporter now uses `snapit-export::export_job::run_export_loop_with_context` for decode-consume loop orchestration (with adapter-owned render context)
  - strengthened `run_export_loop_with_context` callback future bound to `Send` for Tauri command compatibility
  - moved child-process stderr capture + exit-status validation helpers into `snapit-export::process_control`
  - app exporter final FFmpeg wait/error handling now delegates to crate process-control helpers
  - moved frame-stage timing aggregation (rolling decode/gpu/cpu/readback/encode averages) into `snapit-export::timing`
  - app exporter now logs frame timing via crate `FrameTimingAccumulator`
  - moved reusable CPU RGBA frame operations into `snapit-export::frame_ops`:
    - `extract_crop_region`
    - `crop_decoded_frame`
    - `scale_frame_to_fill`
    - `blend_frames_alpha`
  ...
  - moved per-frame CPU cursor overlay compositing orchestration into `snapit-export::cursor_overlay`:
  ...
  - moved staged export loop bookkeeping into `snapit-export::frame_pipeline_state`:
  ...
  - moved shared export finalization helpers into `snapit-export::job_finalize`:
  ...
  - moved shared export drain orchestration into `snapit-export::job_finalize::drain_pipeline_if_needed`:
  ...
  - moved FFmpeg finalize + output-size retrieval into `snapit-export::job_finalize::wait_for_encoder_and_output_size`:
  ...
  - moved completed-export finalization composition into `snapit-export::job_finalize::finalize_completed_export`:
  ...
  - moved base-frame render-mode composition into `snapit-export::frame_composition::build_frame_composition`:
  ...
  - moved per-frame timeline/scene context planning into `snapit-export::frame_context`:
  ...
  - moved per-frame decoded-screen normalization + fast-path planning into `snapit-export::frame_prepare`:
  ...
  - moved per-frame overlay planning into `snapit-export::frame_overlays`:
  ...
  - moved runtime NVENC probe + encoder selection adapter into `snapit-export::encoder_selection`:
...
- Moved shared zoom point-transform math:
  - normalized zoom transform used by exporter cursor compositing + pre-rendered text compositing now centralized in `snapit-render::zoom::apply_zoom_to_normalized_point`
- Moved shared text-overlay quad contract:
  - `TextOverlayQuad` now owned by `snapit-render::types` (instead of app-local `rendering/text_overlay_layer.rs`)
...
- Moved pre-rendered text store/compositing module into shared render crate:
  - `PreRenderedTextStore`, `PreRenderedTextImage`, `LineMetric`, `TextCompositeInfo`, and CPU compositing helpers now live in `snapit-render::prerendered_text`
  - app command/runtime adapters now import those types directly from `snapit-render`
...
- Moved GPU text-overlay renderer module into shared render crate:
  - `TextOverlayLayer` now lives in `snapit-render::text_overlay_layer`
...
- Moved CPU cursor image compositing primitives into `snapit-render`:
  - `DecodedCursorImage`
  - `VideoContentBounds`
  - `composite_cursor`
  - `composite_cursor_with_motion_blur`
  - app `rendering/cursor.rs` now keeps compatibility wrappers while delegating implementation to `snapit-render::cursor_composite`
- Moved shared per-frame cursor geometry planning:
  - crop remap + zoom transform + target-size calculation centralized in `snapit-render::cursor_plan::plan_cursor_geometry`
  - app exporter cursor compositing now consumes crate `CursorGeometryPlanRequest`/`CursorCropPlan` instead of inline math
  - cursor raster fallback decision order (shape SVG -> bitmap -> default arrow) centralized in `snapit-render::cursor_plan::plan_cursor_raster_source`
- Moved shared webcam overlay planning/visibility helpers:
  - `build_webcam_overlay` and `is_webcam_visible_at` centralized in `snapit-render::webcam_overlay`
  - app exporter now imports directly from `snapit-render::webcam_overlay`; obsolete `rendering/exporter/webcam.rs` shim removed
- Moved auto-zoom configuration type ownership into domain crate:
  - `AutoZoomConfig` now lives in `snapit-domain::video_project`
...
- Fixed ts-rs generation target for moved crates:
  - `snapit-domain` and `snapit-render` now export to app canonical path `../../../../src/types/generated/` (from crate module locations)
...
- `cargo test -p snapit-capture --lib` passes (121 tests).
- `cargo test -p snapit-export --lib` passes (95 tests).
- `cargo test -p snapit-render --lib` passes (80 tests).
- `cargo test -p snapit-domain --lib` passes (83 tests).
- `cargo check --workspace --lib` passes.
- `cargo test -p snapit --lib` passes (144 passed, 1 ignored).
...
- Crates should avoid depending on `snapit` (no reverse dependency).
...
1. Define explicit public API surfaces for `snapit-core`, `snapit-domain`, `snapit-media`, `snapit-render`.
...
- `cargo check --workspace --lib` passes.
- `cargo test -p snapit-domain --lib` and `cargo test -p snapit-render --lib` pass.
- No Tauri imports in core/domain/media/render crates.

### Phase 2: Extract Capture Runtime to `snapit-capture`
...
### Phase 3: Extract Export Engine to `snapit-export`
...
### Phase 4: Narrow `snapit` to Integration Layer
...
2. PR 2: Introduce `snapit-capture` and move recording orchestration.
3. PR 3: Expand `snapit-export` from encoder selection to exporter orchestration.
...
- continue reducing exporter-local orchestration in `src/rendering/exporter/mod.rs` (loop + drain + finalize branches) into `snapit-export` while preserving current lifetime-safe adapter boundaries
