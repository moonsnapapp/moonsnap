...
pub fn create_d3d_device() -> windows::core::Result<ID3D11Device> {
    Ok(create_d3d_device_with_warp_fallback()?.0)
}
...
pub enum NewCapturerError {
    ...
    #[error("Windows Graphics Capture API is disabled or unavailable")]
...
impl Capturer {
    pub fn new(
        ...
        if let Some(cursor_capture_enabled) = settings.is_cursor_capture_enabled {
            let _ = session.SetIsCursorCaptureEnabled(cursor_capture_enabled);
        }

        if let Some(min_update_interval) = settings.min_update_interval {
            let _ = session.SetMinUpdateInterval(min_update_interval.into());
        }
...
impl FrameBuffer<'_> {
    ...
    /// Copy frame data to a Vec, removing stride padding
    pub fn to_vec_no_stride(&self) -> Vec<u8> {
        let row_bytes = self.width as usize * 4;
        if self.stride as usize == row_bytes {
            self.data[..row_bytes * self.height as usize].to_vec()
        } else {
            let mut output = Vec::with_capacity(row_bytes * self.height as usize);
