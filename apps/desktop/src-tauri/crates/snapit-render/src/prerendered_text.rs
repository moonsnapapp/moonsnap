...
use snapit_domain::video_project::{TextAnimation, TextSegment};
...
pub struct TextCompositeInfo {
    ...
    pub opacity: f32,
    /// RGBA pixel data (shared Arc â€” zero-copy from store).
    pub rgba_data: Arc<Vec<u8>>,
    ...
    pub fn get_gpu_quads_for_frame(
        ...
        for (idx, image) in &self.images {
            ...
            let (
                ...
                // No clipping â€” fully revealed or no typewriter animation
...
/// No float math at all â€” uses `(src * a + dst * (255-a) + 128) / 255`.
...
fn composite_text_fast(
    ...
    for row in 0..text.dst_h {
        ...
        for col in 0..text.dst_w {
            ...
            // Skip transparent pixels â€” most of the text image is empty.
            if sa_raw == 0 {
                continue;
            }
