...
use snapit_domain::video_project::{ZoomConfig, ZoomRegion, ZoomRegionMode};
...
mod tests {
    ...
    use snapit_domain::video_project::{EasingFunction, ZoomTransition};
    ...
    fn make_interpolator(regions: Vec<ZoomRegion>) -> ZoomInterpolator {
        ZoomInterpolator::new(&ZoomConfig {
            mode: snapit_domain::video_project::ZoomMode::Manual,
            auto_zoom_scale: 2.0,
            regions,
        })
    }
    ...
    #[test]
    fn test_apply_zoom_to_normalized_point() {
        let zoom = ZoomState {
            scale: 2.0,
            center_x: 0.5,
            center_y: 0.5,
        };
        let (x, y) = apply_zoom_to_normalized_point(0.5, 0.5, zoom);
